...
run. It is gitignored — back it up like any other database.
...
| `DATA_DIR` | `./data` | Where `db.json` is written |
...
- **`db.json` is the whole database.** Persist and back up `DATA_DIR`; on
  ephemeral hosts (containers, some PaaS) mount a volume or the content
  resets to defaults on every deploy.
